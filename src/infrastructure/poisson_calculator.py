...
class ScipyPoissonCalculator(ProbabilityCalculator):
    """Calculador matricial Poisson corregido mediante Dixon-Coles."""
    def __init__(self, max_goals: int = 8):
        self.max_goals = max_goals

    def _build_matrix(self, expected_goals: ExpectedGoals) -> np.ndarray:
    ...
    def calculate_distribution(self, expected_goals: ExpectedGoals, top_n: int = 5) -> Tuple[Dict[str, float], List[Dict[str, float]]]:
        matrix = self._build_matrix(expected_goals)
    ...
    def calculate_over_under(self, expected_goals: ExpectedGoals, lines: List[float]) -> List[Tuple[float, float, float]]:
        ...
        for line in lines:
            ...
            results.append((line, float(over_prob), float(under_prob)))
        return results
